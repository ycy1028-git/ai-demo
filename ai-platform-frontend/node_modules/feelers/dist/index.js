...
const feelBlock = new lr.ExternalTokenizer((input, _stack) => {
...
const simpleTextBlock = new lr.ExternalTokenizer((input, _stack) => {
...
const feel = new lr.ExternalTokenizer((input, _stack) => {
...
const buildNodeEvaluator = (options) => {
  ...
  const evaluateNodeValue = (node, context = {}) => {
    ...
    case 'Insert': {
      ...
      try {
        const { value } = feelin.evaluate(`string(${feel})`, context);
    ...
    case 'ConditionalSpanner': {
      ...
      try {
        const { value } = feelin.evaluate(feel, context);
        shouldRender = value;
      }
      catch {
        return errorHandler(new Error(`FEEL expression ${feel} couldn't be evaluated`));
...
const feelersLinter = lint.linter(cmFeelersLinter());
